...
{
	GPNGUtility::InitLogger();

	auto plugin = SKSE::PluginDeclaration::GetSingleton();
	auto gameVersion = REL::Module::get().version();
	logger::info("{} {} version {} is loading into {}", GPNGUtility::logLevelI, plugin->GetName(), plugin->GetVersion(), gameVersion);
	...
	if (papyrusInterface) {
		if (papyrusInterface->Register(GPNGPapyrus::Register)) {
			logger::info("{} Papyrus functions registered", GPNGUtility::logLevelI);
			return true;
		}
		logger::info("{} Could not register Papyrus functions", GPNGUtility::logLevelF);
		return false;
	}
	logger::info("{} Papyrus interface not found", GPNGUtility::logLevelF);
	return false;
}
